...
def create_xg_plot(my_team_df, opp_df, target_team, opp_name):
    ...
    if not MATPLOTLIB_AVAILABLE:
        st.error("Matplotlib is not available. Cannot create visualization.")
...
def main():
    ...
    # Check matplotlib
    if not MATPLOTLIB_AVAILABLE:
        st.error("⚠️ Matplotlib is not properly installed. Charts cannot be displayed.")
        st.info("Please contact the app administrator to fix the deployment.")
    ...
    with st.sidebar:
        ...
        target_season = st.text_input(
            "Season Year",
            value="2019",
            help="Enter season year (e.g., 2019, 2024)"
        )
            ...
            if not target_team or not target_season:
                ...
                with st.spinner(f"Searching for {target_team} matches in {target_season}..."):
                    ...
                    if match_data and len(match_data) > 0:
                        ...
                        st.info("• Use underscores: Manchester_City\n• Try seasons 2014-2024")
    ...
    if st.session_state.match_data:
            ...
            if analyze_button and match_id:
                with st.spinner("⏳ Loading match data..."):
                    ...
                    if len(df_home) == 0 and len(df_away) == 0:
                        st.warning("⚠️ No shot data available")
                        return
                    
                    df_home = clean_df(df_home)
                    df_away = clean_df(df_away)
                    
                    home_team_name = df_home["h_team"].iloc[0] if len(df_home) > 0 else "Home"
                    away_team_name = df_away["a_team"].iloc[0] if len(df_away) > 0 else "Away"
                    ...
                    with col1:
                        st.metric(f"{st.session_state.target_team} Total xG", f"{my_xg:.2f}")
                    with col2:
                        st.metric(f"{opp_name} Total xG", f"{opp_xg:.2f}")
                    with col3:
                        st.metric("xG Difference", f"{abs(xg_diff):.2f}", delta=f"{xg_diff:.2f}")
        ...
        with col2:
            st.markdown("""
            ### 📊 What is xG?
            **Expected Goals** measures chance quality
            - 🔴 Red: Your team
            - 🔵 Blue: Opponent
            - ⭐ Stars: Goals scored
            """)
